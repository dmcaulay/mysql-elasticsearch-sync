...
var nextChunk = function(callback) {
  // get the next batch
  redis.get_and_watch('entry_set_updater', function(err, nextParams) {
    ...
    mysql.db.query(query, function(err, rows) {
      ...
      if (rows.length) {
        ...
        redis.set_watched('entry_set_updater', last.id + ',' + last.updated_at, function(err) {
...
// elastic search
var es = require('node-elasticsearch').avocado.addType('entry_set');
